...
$(FONTDIR)/var/Inter2V.var.ttf: $(FONTDIR)/var/Inter2.var.ttf | venv
	. $(VENV) ; python misc/tools/rename.py --family "Inter2 V" -o $@ $<
$(FONTDIR)/var/Inter2V-Italic.var.ttf: $(FONTDIR)/var/Inter2-Italic.var.ttf | venv
	. $(VENV) ; python misc/tools/rename.py --family "Inter2 V" -o $@ $<

var: \
	$(FONTDIR)/var/Inter2.var.ttf \
	$(FONTDIR)/var/Inter2-Italic.var.ttf \
	$(FONTDIR)/var/Inter2V.var.ttf \
	$(FONTDIR)/var/Inter2V-Italic.var.ttf
...
zip_beta: \
		$(FONTDIR)/var/Inter2V.var.ttf \
		$(FONTDIR)/var/Inter2V.var.woff2 \
		$(FONTDIR)/var/Inter2V-Italic.var.ttf \
		$(FONTDIR)/var/Inter2V-Italic.var.woff2
	mkdir -p build/release
	zip -j -q -X "build/release/Inter2_beta-$(VERSION)-$(shell date '+%Y%m%d_%H%M')-$(shell git rev-parse --short=10 HEAD).zip" $^
...
install_var: \
	$(INSTALLDIR)/Inter2V.var.ttf \
	$(INSTALLDIR)/Inter2V-Italic.var.ttf
